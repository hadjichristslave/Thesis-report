...
The Downsampling and Filtering operations are important as they directly affect the speed of the sampler. Having the downsampling operations run with a low threshold will lead to a large cloud and a small clustering process. On the other hand, having downsampling operations run with a very high threshold can lead to a very small cloud that has a very small amount of information the inital cloud carried. Fig.\ref{downsamplehighandlow} shows the results of having the pipeline run with a high and a low threshold on the milk cartridge dataset. First figure leaves the cloud almost intact whereas the second one only keeps a small part of the points the initial cloud carried. The information loss on clean datasets such as this is not significant but plays a significant role in noise-rich data such as data taken from kinect readings while mapping.


\begin{figure}
\begin{tabular}{cc}
    \includegraphics[width=.5\textwidth]{smalldownsamplinglimit} &  \includegraphics[width=.5\textwidth]{largedownsamplinglimit} \\
    (a) Small threshold in the downsampling leads to & (b) A high threshold in the downsampling leads to  \\
    a high amoung of points propagated to the sampler& a small amount of poits propagated to the sampler \\[6pt]
\end{tabular}
\caption{The behaviour of the dowsampling with respect to the threshold of the operations.}
