...
The model definition is structured as follows: An introduction to the general pipeline that will serve as the compressed sensor model of the EKF slam module is represented first. Every component of the pipeline is then analysed in detail in relation to the operations it serves.
...
Finally, the pipeline was used in real life scenarios as a sensor model in a Landmark based EKF SLAM algorithm and was tested in its speed and memory requirements. In Fig.\ref{slam} an end result of a slam session using the pipeline as a sensor model is shown. The yellow spheres represent the landmarks extracted during the slam session with every sphere representing the environment signature at the specific part of the map; the noisy cloud readings were taken from the kinect sensor mounted on the robot.
...
The memory requirements are shown in Fig.\ref{memReq} as a function of strength parameter alpha. The compression that the method introduces is significant since even for a very high value of alpha, since the memory needs of the landmarks are smaller than 30KB of memory total. Mapping the same environment using the RTAB mapping method, results in maps that average 84MB of memory. It can be seen that there is a significant compression ratio of 1/2800.
