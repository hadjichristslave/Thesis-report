...
In this thesis a method of creating compressed representation of a pointclouds is introduced; the representation's compression strength, expressive ability as well as the ability to handle online streams of data are presented in the results section. The discussion section investigates some details regarding the mechanisms of the method. Cases where the method does not perform as needed as well as the implications of having a method that has an unsupervised learning mechanism at its core are presented. Finally, the motivation behind the object signature and the scalability of the method are discussed.
...
The repesentation introduced is very scalable with respect to its memory needs and the matching operations.
The small amount of memory a single landmark singature requires(260Bytes) leads to a large amount. Fig.\ref{pip:reqs} displays the memory requirements in relation to the number of landmarks the environment has. It can be seen that even for a very large amount of landmarks the memory requirements are low. 
...
\begin{figure}[!ht]
  ...
  \caption{Memory requirements with respect to the number of landmarks in the environment.}
  \label{pip:reqs}
\end{figure}
