...
The downsampling and filtering operations are important as they affect the speed of the sampler. 
During those operations the cloud is transformed from raw readings to the data that the sampler expects as readings.
Downsampling is used to reduce the size of the cloud making the calculations manageable in real-time. Filtering operations calculate the features for every point of the downsampled cloud.
Having the downsampling operations run on a low threshold will lead to a large cloud after the donwsampling, and this will affect the speed of the process.
On the other hand, having downsampling operations run with a very high threshold can lead to a very small cloud that has a very small amount of information the inital cloud carried.
Fig.\ref{pip:downsample} shows the results of having the pipeline run with a high and a low threshold on the milk cartridge dataset as was shown in Fig.\ref{pcl:clust}(a).
The first figure displays a cloud with many points and minimizes the information loss propagated to the sampler; the second one only keeps a small part of the points the initial cloud carried. The information loss on clean datasets such as this is not significant but plays a significant role in noise-rich data such as data taken from kinect readings while mapping.
...
The data distribution was introduced in \ref{data:dist} and was analyzed in the results section and is an extension to the distribution proposed in \cite{dependentDiri}.
