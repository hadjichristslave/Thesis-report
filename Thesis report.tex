...
Given this very dense input point cloud the pipeline described will output its reduced version which is shown in Figure~\ref{pcl:reduced}.


\begin{figure}[h!]
  \centering
    \includegraphics[width=1\textwidth]{initial}
    \caption{Point cloud post pipeline modifications.}
  \label{pcl:reduced}
\end{figure}

As you can see the cloud structure remains intact and there is no loss of structural information. The cloud is now ready to be used as input for the sampler which will take the features calculated in the pipeline to cluster the cloud into segments of structure.
