...
\textbf{Lines 6-12:} The correspondence of the previous landmarks to current ones happens in theses lines.The getBestLandmarkCorrespondence function is a random forest implementation. An offline model has been trained to recognize correlation between distances and data correspondence. This approach is similar to~\cite{objectDisc} and many the efficiency of the method depends on the training to be general enough. The random forest outputs the probability of a landmark having been encountered before; if it has, it is being added to the landmarks list to be send for update in the EKF, otherwise a new landmark is added and its ID is then updated to the list.

\textbf{Lines 15:} The algoritm returns the list of landmarks given the pointcloud and all the previous landmarks the robot has encountered.
