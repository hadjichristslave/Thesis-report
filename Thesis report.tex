...
Once the sampler has ouput the clusters of the point cloud. The decision of how an observation relates to previously encountered data has to be made. This decision is taken in the decision layer of the algorithm. For ease of use and due to time restrictions, a simple decision layer was implemented. The question that the decision layer answers is: "How does the data I currently see relate to the data I have previously seen in the past?". To do that, distance measures must be defined between the observations and the landmarks. Distances between distributions are called divergences and a large amount of literature on divergences exists.

Since the distributions output consist of 3 separate parts, specific divergences for each part were considered. More specifically let $l$ be the distribution of the landmark and $o$ the distribution of the observation. $l$ can be decomposed into 3 parts: $l_G$,$l_C$,$l_E$ where G,C and E stand for Gaussian, Categorical and Exponential respectively. The observation distribution can be decomposed similarly. With that notation the distances between those distributions can be defined. For each individual landmark distribution $l$ and observation distribution $o$ the distances computed where the following:
...
Empiric evaluation and some parameter tweaking showed a nice splitting boundary between landmarks. It is of course possible to learn the optimal decision boundary but optimizing the decision layer was outside the scope of this project.
...
\begin{figure}
\begin{tabular}{cc}
  \includegraphics[width=60mm]{robotPictures/experiment1/input} &   \includegraphics[width=85mm]{robotPictures/experiment1/clustersWithData} \\
(a) Input  & (b) Pointcloud along with landmarks\\[6pt]
 \includegraphics[width=60mm]{robotPictures/experiment2/depthinput} &   \includegraphics[width=85mm]{robotPictures/experiment2/clustered} \\
