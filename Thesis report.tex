...
Now that the data distribution and the GPU are defined, the pipeline and its operating mechanisms can be presented. Fig.~\ref{pipeline}(a) shows the flow of operations. The EKF-SLAM module requests new observation readings given the point cloud currently read by the sensors. The pipeline takes that cloud, extracts clusters and returns the landmarks currently being observed.  Fig.~\ref{pipeline}(b) shows a more detailed view of what happens in the pipeline. The same procedure is described in Algorithm~\ref{euclid}.
